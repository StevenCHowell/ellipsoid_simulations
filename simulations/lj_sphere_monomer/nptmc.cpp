...
void lj_energy(sasmol::SasMol &mol, float &x1, float &y1, float &z1, int &thisi, float &box_length,float &v12, float &v6, float &w12, float &w6){
	...
	float x2,y2,z2 ;
	float sr2,sr6,vij12,vij6,rcutsq ;
	float rxij,ryij,rzij,rijsq ;

	v12 = 0.0, v6 = 0.0, w12 = 0.0, w6 = 0 ;

	rcutsq = pow((box_length/2.0),2.0) ;
...
void sumup(sasmol::SasMol &mol,RunParameters &par,float &v12,float &v6,float &w12,float &w6){

	int frame = 0 ;
	float rcutsq = par.rcut * par.rcut ;
	...
	for(int i = 0 ; i < mol._natoms() - 1 ; ++i)
	{
		...
		for(int j = i + 1 ; j < mol._natoms() ; ++j)
		{
			...
			rxij = rxij-par.box_length*(round(rxij*par.boxlinv)) ;
			ryij = ryij-par.box_length*(round(ryij*par.boxlinv)) ;
			rzij = rzij-par.box_length*(round(rzij*par.boxlinv)) ;
...
//	densityfile=open(runname+'_density.dat','w')
	...
	par.rcut = 0.5*par.box_length ;
	std::cout << "cutoff (rcut) = " << par.rcut << std::endl ;
	
	par.ljradius = (par.diameter_of_Ar/2.0) / par.sigmaAr ;

	float dboxmx = par.box_length/40.0 ;
	float drmax = par.delta_translation ;

	float beta = 1.0/par.temperature ; 		// i.e. kb=1.0
	float sr3 = pow((1.0/par.rcut),3.0) ;
	...
	float molvolume = ((mol._natoms()*4.0*M_PI*(pow(par.ljradius,3.0)))/3.0) ;
...
//     rxiold.setZero(mol._natoms(), 1);
	...
	//				std::cout << std::endl << "boltz acc" << std::endl ;
		...
		par.eta=molvolume/pow(par.box_length,3.0) ;
...
//		boxfile.flush() ; pressfile.flush() ; densityfile.flush()
		...
		if(fmodf(float(this_step),(par.number_of_steps/float(100.0)))==0)
		{
			...
			{
				std::cout << "\nSTEP\t\tVN\t%ACC_MOV\t%ACC_VOL\t ETA\t\t<ETA>\t\tDENSITY\t\t<DENSITY>\tPRESSURE\t<PRESSURE>\tBOXL\n" << std::endl ;
				std::cout << this_step << " "<<vn <<" "<< acatma*100.0/m <<" "<< acboxa*100.0/v <<" "<< par.eta <<" "<< sumeta/tv <<" "<< par.density <<" "<< acd/(tm+tv) <<" "<< par.pressure <<" "<< acp/(tm+tv) <<" "<< par.box_length << std::endl ;
			}
