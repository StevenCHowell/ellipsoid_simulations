...
void
sascalc::cuda::
wrapper_cudaGV_calcIq_Merge_GVVV(const float * const gpu_coor, const float * const gpu_b, const float * const gpu_radius, const float * const gpu_gv, const float * const gpu_q, float * const gpu_Is, const int Natoms, const int Nq, const int Ngv, const float sld_solvent, const float scale_volume)
{
	// set the CUDA block dimenstions
    dim3 dim_grid(Ngv), dim_block(Nq);
	// cudakernel launch configuration
