...
{
    // initialization
    const float delta = 2.0; // ZHl hardwired
    ...
    float volume = (delta*delta*delta*count_filled_grid)*(float(_Npoints)/float(*cpu_Npoints_tried));
...
send()
{
    CALL_CUDA(cudaMemcpy(_gpu_points, _points, _Npoints*3*sizeof(float), cudaMemcpyHostToDevice));
}
