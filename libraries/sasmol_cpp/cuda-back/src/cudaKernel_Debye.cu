...
/// @par Main functionality
/// Calculate I(q) based on Debye summation for a CustomizedModel
/// @param [in] gpu_coor the XYX coordinates where the leading dimension is Natoms and the slow dimension is X->Y->Z
/// @param [in] gpu_Ia the partial intensity where the leading dimension is Natoms and the slow dimension is Nq
__global__ void
sascalc::cuda::
cudaKernel_Debye_calc_CustomizedModel(const float * const gpu_coor, const float * const gpu_b, const float * const gpu_radius, const float * const gpu_q, float * const gpu_Ia, const int Natoms, const int Nq)
{
	// get the thread id and return if necessary
    const int idx = blockIdx.x*blockDim.x + threadIdx.x;
    if (idx>=Natoms) return;
    ...
    __shared__ float shared_z[BLOCKDIM];
    __shared__ float shared_b[BLOCKDIM];
    __shared__ float shared_radius[BLOCKDIM];
    ...
    const float zi = gpu_coor[Natoms*2+idx];
    const float bi = gpu_b[idx];
    const float radius_i = gpu_radius[idx];

    // local variables
    int idx_atom,idx_block, idx_q;
    float r,q,qr,bj,radius_j;

    // for atom itself
    for (idx_q=0; idx_q<Nq; ++idx_q)
    {
        q = gpu_q[idx_q];
        gpu_Ia[idx_q*Natoms + idx] = bi*bi*exp(-0.5/M_PI*q*q*powf(0.75*M_PI, 2./3.)*radius_i*radius_i);
    }
    ...
    for (idx_block=blockIdx.x; idx_block<gridDim.x; ++idx_block)
    {
        ...
        shared_z[threadIdx.x] = gpu_coor[Natoms*2 + idx_atom];
        shared_b[threadIdx.x] = gpu_b[idx_atom];
        shared_radius[threadIdx.x] = gpu_radius[idx_atom];
        __syncthreads();
        for (idx_atom=0; idx_atom<blockDim.x; ++idx_atom)
        {
            if (idx_block==blockIdx.x && idx_atom<=threadIdx.x) continue;
            ...
            r = sqrt ( powf(xi-shared_x[idx_atom], 2.0) + powf(yi-shared_y[idx_atom], 2.0) + powf(zi-shared_z[idx_atom], 2.0) );
            bj = shared_b[idx_atom];
            radius_j = shared_radius[idx_atom];
            for (idx_q=0; idx_q<Nq; ++idx_q)
            {
                q = gpu_q[idx_q];
                qr = q*r;
                if (qr==0.0) gpu_Ia[idx_q*Natoms + idx] += 2.*bi*bj*exp(-0.25/M_PI*q*q*pow(0.75*M_PI, 2./3.)*(radius_i*radius_i+radius_j*radius_j));
                else gpu_Ia[idx_q*Natoms + idx] += 2.*bi*bj*exp(-0.25/M_PI*q*q*pow(0.75*M_PI, 2./3.)*(radius_i*radius_i+radius_j*radius_j))*sinf(q*r)/(q*r);
                //if (idx>=1958 && idx_q==0) printf("self: %04d, other: %04d, qr: %f, bi: %f, bj: %f, gpu_Ia[idx_q*Natoms + idx]: %f\n",idx,idx_block*blockDim.x + idx_atom,qr,bi,bj,gpu_Ia[idx_q*Natoms + idx]);
            }
         }
    }
