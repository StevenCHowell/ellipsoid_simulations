...
namespace sasmol
{
    ...
    class SasAtom: public sasio::Files, public sascalc::Prop, public sasop::Move, public sasmath::Math, public sassubset::Mask
    {
        ...
        public:
            ...
            virtual Eigen::Array<float,Eigen::Dynamic,Eigen::Dynamic> _coor(const int frame = 0) { Eigen::Array<float,Eigen::Dynamic,Eigen::Dynamic> coor(natoms,3); coor<<x.col(frame),y.col(frame),z.col(frame); return coor; } ///< (Brief description)
            virtual std::vector<Eigen::Vector3f> _vcoor(const int frame = 0) { std::vector<Eigen::Vector3f> vcoor; for (int i=0; i<natoms; ++i) vcoor.push_back((Eigen::Vector3f()<<x(i,frame),y(i,frame),z(i,frame)).finished()); return vcoor; } ///< (Brief description)
