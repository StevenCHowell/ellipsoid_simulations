...
class RunParameters{
    ...
    public:
        ...
        std::string runname = "run1" ;
        const std::string dcd_output_filename = "output/run1.dcd" ;

    const int dcd_save_frequency = 10 ;               // how often to save frames
    const std::string input_filename = "../../p_0200.pdb" ;  // input pdb
    const std::string output_filename = "output/run1.pdb" ; // output pdb (last frame)
    ...
    float goal_pressure = 0.18 ;                       // pressure (unitless) <--- ??? do not believe these units ???
