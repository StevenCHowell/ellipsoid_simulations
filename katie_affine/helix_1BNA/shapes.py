...
def hollowCylinder_equation(x,y,z,radius=1.0,inner_radius=0.24,half_length=1.0): ##ZHL note: affine transformation doesn't alter ro/ri; inner_radius is hardwired for 1UTD
    '''
    Check whether a point are within a unit hollowClinder or not
    Return: True if inside or on surface, False if outside
    '''
    if z>=-half_length and z<=half_length and math.sqrt(x*x+y*y) <= radius and math.sqrt(x*x+y*y) >= inner_radius:
        return True
    else:
        return False
